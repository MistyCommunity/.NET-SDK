...
namespace MistySkillTypes
{
	public class ChargerDock
	{
		...
		private readonly double _headPitchOffset;
		private readonly double _headRollOffset;
		private readonly double _headYawOffset;
		...
		private const float CENTER_OFFSET = 0.035f;					// Camera for dock position is to the right of Misty's center
		private const double ALIGNED_X = 0.02;						// Tolerance for considering us aligned well enough to dock
		private const double ALIGNED_EULER_YAW = 4;                 // Tolerance for considering us aligned well enough to dock
		private const double FINAL_OFFSET_ANGLE_MAX = 10;			// Tolerance for final alignment angle check
		...
		public ChargerDock(IRobotMessenger misty, SkillHelper skillHelper, 
		...
		private async Task<bool> FaceChargerAsync(double xTolerance)
		{
			try
			{
				...
				while (Math.Abs(_chargerPose.X) > xTolerance)
				{
					...
					if (Math.Abs(chargerBearing) <= 3.0)
					{
						if (chargerBearing < 0)
							chargerBearing = -3.1;
						else
		...
		private void SlamStatusCallback(ISlamStatusEvent eventResponse)
		{
			...
			foreach (var status in _slamStatus.StatusList)
				s += status + "|";
			_skillHelper.LogMessage($"SLAM sensor status is {_slamStatus.SensorStatus}. SLAM status list is {s}.");

			if (!_chargeDetectorRunning && _slamStatus.StatusList.Contains("DockingStationDetectorEnabled") && _slamStatus.StatusList.Contains("Streaming"))
			{
	...
	internal class ChargerPosition
	{
